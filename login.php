...
if (isset($_POST['next2'])) {
	...
	mysqli_query($db, "INSERT INTO member(lastname,firstname,gender,userid,password,email,contactno,birthdate,address,verification,memberimg) VALUES ('$lname','$fname','$gender','$userid','$password','$email','$contactno','$birthdate','$cusaddress','no','default.jpg')");
	$query = mysqli_query($db, "SELECT * FROM member WHERE userid = '$userid'");
	$row = mysqli_fetch_array($query);
	$id = $row['memberid'];
	mysqli_query($db, "INSERT into secretquestions(memberid,question,answer) VALUES ('$id','$secques','$secanswer')");
	$_SESSION['ID'] = $id;
...
if (isset($_POST['login'])) {
	if (isset($_POST['user'])) {
		if (isset($_POST['pass'])) {
			$username = $_POST['user'];
			$pass = $_POST['pass'];
			$query = mysqli_query($db, "SELECT * FROM member WHERE userid = '$username' AND  password = '$pass'");
...
<div id="main_content">
	...
	<div class="left_content">
		...
		<ul class="left_menu">
			...
			<div class="banner_adds"> <a href="#"><img src="images/ewu.jpg" alt="" border="0" /></a> </div>
	</div>
	<!-- end of left content -->
